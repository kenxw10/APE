...
9g. Market worker persistence backpressure hardening. Current PR.
...
Next manual checkpoint after PR 9g: validate only `ape-market-worker` using `python -m ape.worker --role market-data` before enabling reference, strategy, or maintenance workers. Confirm `/ws/status` uses `liveness_source=component` from `ape-worker.market_data`, shows `worker_role=market-data`, `connection_state=subscribed`, `subscription_reconciled=true`, `orderbook_sid_confirmed=true`, healthy transport/subscription state, low critical queue depth/age, recent `latest_state_persisted_at`, bounded `latest_state_persisted_age_ms`, and no repeated `market_critical_persistence_backpressure`, `market_critical_persistence_failed`, `market_db_writer_queue_backpressure`, or `orderbook_persistence_pending` hard blockers. Diagnostic protocol events may be sampled or dropped under load, but that must not block readiness. After market worker validation passes, then deploy `ape-reference-worker`, `ape-strategy-worker`, and `ape-maintenance-worker` with their matching role commands. Keep `APP_MODE=DRY_RUN`, `STRATEGY_OBSERVER_ENABLED=true`, `STRATEGY_DRY_RUN_ENABLED=true`, `TRADING_ENABLED=false`, and `EXECUTE=false` on the strategy worker; keep `TRADING_ENABLED=false` and `EXECUTE=false` everywhere. The API and dashboard may stay read-only; Vercel must not receive Kalshi credentials, WebSocket variables, BRTI env vars, strategy env vars, storage retention env vars, dry-run controls, private-channel controls, account reads, or order/execution controls.
