...
Next manual checkpoint after PR 9e: keep market WebSocket, BRTI, strategy observer, and storage retention worker settings healthy; keep `APP_MODE=DRY_RUN`, `STRATEGY_OBSERVER_ENABLED=true`, `STRATEGY_DRY_RUN_ENABLED=true`, `TRADING_ENABLED=false`, and `EXECUTE=false` on the Railway worker only; redeploy the worker; and validate worker logs, `/strategy/dry-run/status`, `/strategy/dry-run/positions/open`, `/strategy/dry-run/positions/recent`, `/strategy/dry-run/events/recent`, `/strategy/status`, `/strategy/decisions/latest`, `/strategy/decisions/recent`, `/strategy/gates/recent`, `/storage/status`, `/ws/status`, `/reference/brti/status`, `/reference/brti/latest`, `/reference/brti/series`, `/health`, `/safety`, `/db/status`, `/ready`, `/kalshi/status`, and `/markets/active`. Confirm `/ws/status` uses `liveness_source=component` from `ape-worker.market_ws`, `/reference/brti/status` uses `liveness_source=component` from `ape-worker.reference_brti`, and strategy decision measurements show component liveness sources without `feed_liveness_legacy_aggregate_fallback`. Confirm market decisions expose transport/subscription/snapshot/ticker/sequence state, quiet-data warnings, snapshot source, recovery action, `market_feed_state`, recovery counts, last recovery reason/action/result, and recovery attempt age. Confirm `KALSHI_STALE` means true market feed failure, unrecovered subscription/snapshot recovery failure, or carry-forward cap breach, not quiet healthy data or a bounded pending recovery. Confirm BRTI remains stricter: missing valid BRTI ticks still produce `REFERENCE_STALE` and reconnect diagnostics. The API and dashboard may stay read-only; Vercel must not receive Kalshi credentials, WebSocket variables, BRTI env vars, strategy env vars, storage retention env vars, or dry-run controls.
