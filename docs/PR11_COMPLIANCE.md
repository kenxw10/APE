...
| R3 isolated worker and public reconciliation | `src/ape/worker/main.py`, `src/ape/research/service.py` | `test_r3_worker_roles_keep_research_isolated_and_market_data_owns_reconciliation`; `test_r3_reconciler_is_public_only_and_official_result_wins`. |
...
| R5 zero-entry funnel and frequency classes | `src/ape/research/replay.py` | `test_r5_funnel_frequency_classifications_are_explicit` covers all funnel stages and all six fill-frequency classifications. |
...
| R13 bounded read-only APIs and status | `src/ape/api/main.py`, `src/ape/research/status.py` | `tests/test_research_api.py`, including `test_zero_entry_route_returns_bounded_database_error`, `test_r13_research_api_surface_is_read_only_and_bounded`, and smoke's research/storage read-route map. |
