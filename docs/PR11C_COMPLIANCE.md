...
| R6 failure and restart semantics | Cursor and archive writes share the commit boundary; a real empty bootstrap window advances durably under the one-operation budget, source retention below a committed cursor does not regress TAIL state, duplicate retry rolls back flushed cursor/archive mutations before a fresh-session retry, and a sanitized timeout heartbeat is followed by resumable progress. | `test_empty_bootstrap_window_is_durable_work_and_counts_against_budget`; `test_source_retention_below_committed_cursor_does_not_regress_tail`; `test_duplicate_retry_rolls_back_mutated_cursor_and_archive_then_retries`; `test_timeout_is_sanitized_and_next_cycle_can_resume_cursor_archive`. |
| R7 direct behavioral coverage | The PR-specific suite exercises production SQL shape, a genuinely empty bootstrap window, durable window advancement, operation-budget accounting, source retention below a committed cursor, tail recovery for a new higher row, flushed duplicate rollback, fresh-session retry, bootstrap gaps, frozen-target tail rows, pending reads, timeout recovery, all six cursor sources, and the unchanged market boundary. | `tests/test_pr11c_scope_contract.py` (18 direct behavioral tests), including the three exact R6 proofs above. |
...
- `python -m pytest tests/test_pr11c_scope_contract.py tests/test_research_archive.py tests/test_research_runtime.py tests/test_research_worker.py tests/test_pr11b_scope_contract.py -q`: 75 passed.
...
- `python -m pytest`: 578 passed, 284 warnings in 282.37 seconds.
